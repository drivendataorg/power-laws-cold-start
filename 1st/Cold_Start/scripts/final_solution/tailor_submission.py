...
from coldstart.keras.model import MetaModel, load_custom_model
from coldstart.keras.data import prepare_x
from coldstart.utils import load_data

def main():
    model_dirs = glob.glob(
        DATASET_PATH + '/models/tailor/*')
    if not os.path.exists(os.path.join(DATASET_PATH, 'submissions', 'tailor')):
        os.makedirs(os.path.join(DATASET_PATH, 'submissions', 'tailor'))
...
def create_submission(model_dir):
    submission_path = os.path.join(
        DATASET_PATH, 'submissions', 'tailor', '%s.csv' % os.path.basename(model_dir))
    ...
    test_preds = {}
    submission_series_id = submission.series_id.unique()
    for series_id in submission_series_id:
        ...
        x = prepare_x(window, df, metadata, series_id)
        pred = meta_model.predict(x, window)
        test_preds[series_id] = pred[0]
